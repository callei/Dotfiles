#!/usr/bin/env bash
# ------------------------------------------------------
# System Update Script, from ml4w (Stephan Raabe)
#    ____         __       ____               __     __
...
# Trap escape key (SIGINT) to exit cleanly
trap "echo; echo ':: Update canceled.'; exit" SIGINT

# Default colors if config files don't exist
primarycolor="#FFFFFF"
onsurfacecolor="#000000"

# Import colors from current theme
if [ -f ~/.config/themes/current.css ]; then
    # Extract hex codes from css file
    primarycolor=$(grep "@define-color accent" ~/.config/themes/current.css | awk '{print $3}' | tr -d ';')
    onsurfacecolor=$(grep "@define-color fg" ~/.config/themes/current.css | awk '{print $3}' | tr -d ';')
fi

clear
gum style --align center "$(figlet -f smslant "Updates")"
echo
