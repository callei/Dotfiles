...
Item {
    ...
    property bool isReady: visible && localFolderModel.status === FolderListModel.Ready && !window.isClosing
    ...
    function applyWallpaper(safeFileName, isVideo) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    (
                        ...
                        export WPCTL="${escapeBash(window.wpctlPath)}"
                        
                        cp "$DEST_FILE" /tmp/lock_bg.png || true
                        pkill mpvpaper || true

                        "$WPCTL" apply "$DEST_FILE" >/dev/null 2>&1 || true
        ...
        const escWpctl = escapeBash(window.wpctlPath);
        ...
        if (isVideo) {
            ...
            wallpaperCmd = `"$WPCTL" apply "$WALL_FILE" >/dev/null 2>&1`
    ...
    function beginCloseAnimation() {
        ...
        closeAfterApplyTimer.restart()
    }

    Timer {
        id: closeAfterApplyTimer
        interval: 640
        repeat: false
    ...
    property string srcDir: Quickshell.env("HOME") + "/Bilder/wallpapers"

    readonly property var transitions: ["grow", "outer", "any", "wipe", "wave", "pixel", "center"]

    Process {
        id: wallpaperDirProbe
        command: [
            "bash",
            "-c",
            "if [ -d \"$HOME/Bilder/wallpapers\" ]; then echo -n \"$HOME/Bilder/wallpapers\"; " +
            "elif [ -d \"$HOME/Images/Wallpapers\" ]; then echo -n \"$HOME/Images/Wallpapers\"; " +
            "else echo -n \"$HOME/Bilder/wallpapers\"; fi"
        ]
        running: true
        stdout: SplitParser {
            onRead: data => {
    ...
    Shortcut {
        ...
        onActivated: Quickshell.execDetached(["bash", "-c", "echo 'close' > /tmp/qs_widget_state"])
    }
    ...
    ListView {
        ...
        clip: false 
        displayMarginBeginning: window.itemWidth * 2
        displayMarginEnd: window.itemWidth * 5

        interactive: !window.isScrollingBlocked && !window.isApplying
        cacheBuffer: Math.max(window.itemWidth * 14, 6000)
            ...
            Item {
                ...
                Item {
                    ...
                    Image {
                        ...
                        sourceSize: Qt.size(Math.max(1, Math.round(width * 1.5)), Math.max(1, Math.round(height * 1.5)))
    ...
    Rectangle {
        ...
        anchors.topMargin: window.isReady ? (-(window.itemHeight / 2) - window.s(74)) : (-(window.itemHeight / 2) - window.s(170))
        opacity: window.isReady ? 1.0 : 0.0
